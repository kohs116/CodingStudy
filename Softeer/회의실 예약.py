...
# 회의가 가능한 시간은 09 ~ 18 사이이고 최소 1시간 간격이므로 1시간 간격으로 시간 생성
for __ in range(N):
...
for _ in range(M):
    ...
    # 입력받은 시작 시각 ~ 종료 시각 사이에 있는 1시간 간격의 시간들을 예약 가능한 시간에서 제외시킴
    ...
    if len(conference_room[value]) == 0:  # 회의실 예약이 불가능한 경우
        ...
        for i in range(len(conference_room[value])):
            ...
            if i == len(conference_room[value]) - 1:  # IndexError를 방지하기 위해 따로 선언
                ...
                if conference_room[value][i][1] == conference_room[value][i + 1][
                    0]:  # 현재 회의가 끝나는 시간이랑 다음 회의가 시작하는 시간이랑 같을 때
                    tmp.append(conference_room[value][i])
